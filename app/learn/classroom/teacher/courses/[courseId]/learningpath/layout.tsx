...
interface ClassroomLearningPathContextType {
  modules: Module[];
...
const ClassroomLearningPathContext = createContext<ClassroomLearningPathContextType | null>(null);

export function useClassroomLearningPath() {
  const context = useContext(ClassroomLearningPathContext);
  if (!context) {
    throw new Error('useClassroomLearningPath must be used within ClassroomLearningPathProvider');
  }
  return context;
}

export default function TeacherClassroomLearningPathLayout({
  children,
}: {
  ...
  // Handle lessonId change from URL
  useEffect(() => {
    if (!lessonId) return;
  ...
  const fetchModules = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (lessonId) {
          const moduleWithLesson = modulesData.find((m: Module) => 
  ...
  const handleLessonClick = useCallback((lessonId: string) => {
    ...
    router.push(`/learn/classroom/teacher/courses/${courseId}/learningpath/lesson/${lessonId}`);
  }, [lessonCache, router, courseId, currentLessonContent]);

  // Teacher doesn't save progress - only update local state
  ...
  const contextValue: ClassroomLearningPathContextType = {
    modules,
    selectedLesson,
